    ...
    >
      <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-display font-black text-slate-900 tracking-tight italic">Cierre de Consentimiento</h2>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Podología - Finalización</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
           {initialData && (
              <button 
                ...
                className="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2"
              >
           ...
           <button 
              ...
              >
                ...
                <span>{isSaving ? 'Guardando...' : 'Finalizar Cierre'}</span>
           </button>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto space-y-8 pb-32">
          <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-10">
            <div className="flex flex-col items-center text-center">
               ...
               <h1 className="text-3xl font-display font-black text-slate-900 italic">MEDICAL <span className="text-brand-purple">D'LIS</span></h1>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-2">Cierre de Acta Podológica</p>
            </div>

            <div className="grid grid-cols-2 gap-8 pt-8 border-t border-slate-50">
               <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Nombre del Paciente</label>
               ...
               <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Fecha de Cierre</label>
            ...
            <div className="space-y-1">
               <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Lugar y Fecha</label>
               <input 
                 type="text" 
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-3 text-brand-purple">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Resumen de Tratamiento</h3>
               </div>
               <input 
                 type="text" 
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-3 text-brand-purple">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Observaciones y Recomendaciones Finales</h3>
               </div>
               <textarea 
                 value={formData.observations}
                 onChange={e => setFormData({...formData, observations: e.target.value})}
                 placeholder="Indique los cuidados post-tratamiento o citas de seguimiento..."
                 className="w-full p-4 bg-white border-2 border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:border-brand-purple outline-none min-h-[150px] resize-none"
               />
            </div>
            ...
            <div className="space-y-8 py-8 border-t border-slate-100">
               <div className="space-y-4">
                  ...
                  <ul className="space-y-3">
                     {[
                       ...
                       "Conocer la identidad y calificación del podólogo tratante.",
                       "Solicitar aclaraciones antes, durante y después del procedimiento.",
                       "Revocar su consentimiento en cualquier momento, antes o durante el tratamiento, sin penalización alguna.",
                       "Acceder a la confidencialidad de su información personal y clínica."
                     ].map((text, idx) => (
                        <li key={idx} className="flex gap-3 text-xs font-bold text-slate-500 leading-relaxed">
                           <span className="w-2 h-2 bg-brand-purple rounded-full mt-1.5 shrink-0" />
                           {text}
                        </li>
                     ))}
                  </ul>
               ...
               <div className="space-y-4">
                  ...
                  <p className="text-xs font-bold text-slate-500 leading-relaxed bg-slate-50 p-4 rounded-2xl border border-slate-100">
                     Menciona el tratamiento de datos conforme a la Ley Federal de Protección de Datos Personales en Posesión de Particulares. 
                     Los datos no serán compartidos sin autorización expresa, salvo requerimiento legal o sanitario.
                  </p>
               </div>

               <div className="space-y-4">
                  <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest underline decoration-brand-purple decoration-4 underline-offset-4">VIII. DECLARACIÓN DE CONSENTIMIENTO</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                     <div className="space-y-1">
                        ...
                        <input 
                           ...
                           className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-xl text-xs font-bold text-slate-900 focus:border-brand-purple outline-none" 
                        />
                     </div>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-slate-100">
               <div className="space-y-4">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Firma del Paciente</p>
                  ...
                  <p className="text-[10px] font-black text-slate-400 text-center uppercase tracking-tighter">{formData.patientName}</p>
               </div>
               <div className="space-y-4">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Firma del Especialista</p>
                  <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-4 relative h-40 flex items-center justify-center overflow-hidden group">
                     {formData.specialistSignature ? (
                     ...
                     {!formData.specialistSignature && (
                        <button onClick={() => specialistSigCanvas.current?.clear()} className="absolute bottom-2 right-2 text-[9px] font-black text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity">Limpiar</button>
                     )}
                  </div>
                  <p className="text-[10px] font-black text-slate-400 text-center uppercase tracking-tighter">Especialista Podológico</p>
               </div>
            </div>
          </div>
        </div>
